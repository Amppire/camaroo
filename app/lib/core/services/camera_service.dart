...
class CameraApiModel implements CameraApi {
  CameraApiModel({required StorageService storageService})
  ...
  Future<void> _updateCameraForFocalLength(double targetFocalLengthMm) async {
    ...
    try {
      if (kDebugMode) {
        print(
          '🎯 Setting focal length: ${targetFocalLengthMm.toStringAsFixed(1)}mm',
        );
      }

      // Step 1: Find best camera for target focal length
      final bestCamera = _findBestCameraForEffectiveFocalLength(
        targetFocalLengthMm,
      );
      ...
      if (needsSwitch) {
        if (kDebugMode) {
          ...
          print(
            '   To: ${bestCamera.name} (${bestBaseFocalLength.toStringAsFixed(1)}mm base)',
          );
        }
      ...
      final clampedDigitalZoom = requiredDigitalZoom.clamp(
        minDigital,
        maxDigital,
      );
      ...
      if (kDebugMode) {
        ...
        print(
          '   Required digital: ${requiredDigitalZoom.toStringAsFixed(2)}x',
        );
      ...
      if (kDebugMode) {
        print(
          '✅ Applied: ${targetFocalLengthMm.toStringAsFixed(1)}mm → ${bestCamera.name} @ ${clampedDigitalZoom.toStringAsFixed(2)}x digital',
        );
      }
    } catch (e) {
      if (kDebugMode) {
    ...
    try {
      ...
      for (final camera in _cameras) {
        ...
        if (props == null) {
          try {
            props = allProps.values.firstWhere(
              (p) =>
                  p.cameraId.contains(camera.name.split(':').last) ||
        ...
        if (kDebugMode) {
          ...
          print(
            '   Focal Length: ${props.focalLength?.toStringAsFixed(2) ?? "N/A"} mm',
          );
      ...
      if (kDebugMode) {
        ...
        for (final entry in _cameraProperties.entries) {
          print(
            '   ${entry.key}: FL=${entry.value.focalLength?.toStringAsFixed(2) ?? "N/A"}mm',
          );
        }
        print('📍 Focal length stops: $_focalLengthStops');
        print(
          '📏 Focal length range: ${_minFocalLength?.toStringAsFixed(1) ?? "N/A"}mm - ${_maxFocalLength?.toStringAsFixed(1) ?? "N/A"}mm',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  CameraDescription? _findBestCameraForEffectiveFocalLength(
